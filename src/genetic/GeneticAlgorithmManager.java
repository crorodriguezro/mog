...
import evaluation.DurationEvaluator;
import javafx.util.Pair;
import model.Resource;
import model.Schedule;
import model.Task;
import scheduling.greedy.Greedy;
...
//        Greedy greedy = new Greedy(evaluatedSchedule.getSuccesors());
    ...
    private Task[] deepCopyOf(Task[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            Task task = array[i];
            resultArray[i] = new Task(task.getId(), task.getRequiredSkills(), task.getDuration(), task.getStart(), task.getSuccessors().clone(), task.getResourceId());
        }
        return resultArray;
    }
